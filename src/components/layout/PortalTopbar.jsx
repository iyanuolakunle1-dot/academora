  ...
  return (
    <header className="sticky top-0 z-30 flex items-center justify-between gap-4 border-b app-border app-surface px-4 py-4 sm:px-6">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        <ThemeToggle compact />
        <Link to={notificationsHref} className="relative rounded-full p-2 text-app-secondary hover:bg-black/5 dark:hover:bg-white/5">
        ...
        <Link to={messagesHref} className="relative hidden rounded-full p-2 text-app-secondary hover:bg-black/5 dark:hover:bg-white/5 sm:block">
        ...
        <div className="relative" ref={ref}>
          <button onClick={() => setOpen((o) => !o)} className="flex items-center gap-2 rounded-full pl-1 pr-2 py-1 hover:bg-black/5 dark:hover:bg-white/5">
            ...
            <span className="hidden text-left sm:block">
              ...
              <span className="block text-[10px] text-accent-600 dark:text-accent-400 font-mono uppercase tracking-wider">
                {profile?.role || 'Staff'}
              </span>
            </span>
            ...
            {open && (
              ...
              >
                <div className="px-3 py-2">
                  <p className="text-xs font-bold text-app-primary truncate">{profile?.full_name || 'Portal User'}</p>
                  <p className="text-[11px] text-app-secondary font-mono truncate">{profile?.email}</p>
                  <span className="inline-block mt-1 text-[10px] font-bold uppercase tracking-wider text-accent-600 dark:text-accent-400 bg-accent-50 dark:bg-accent-950/60 px-2 py-0.5 rounded-full">
                    {profile?.role || 'Staff'} Access
                  </span>
                </div>

                <div className="py-1.5">
                  <Link to={profileHref} onClick={() => setOpen(false)} className="flex items-center gap-2 rounded-xl px-3 py-2 text-xs text-app-primary hover:bg-black/5 dark:hover:bg-white/10 font-medium">
                    <FiUser className="h-3.5 w-3.5 text-app-secondary" /> Portal Profile
                  </Link>
                  <Link to={settingsHref} onClick={() => setOpen(false)} className="flex items-center gap-2 rounded-xl px-3 py-2 text-xs text-app-primary hover:bg-black/5 dark:hover:bg-white/10 font-medium">
                    <FiSettings className="h-3.5 w-3.5 text-app-secondary" /> System Settings
                  </Link>
                </div>
